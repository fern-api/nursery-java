...
import com.fern.nursery.api.client.owner.errors.OwnerNotFoundError;
...
@JsonDeserialize(
    using = GetTokensForOwnerException.Deserializer.class
)
public final class GetTokensForOwnerException extends Exception {
  private final Value value;

  private int statusCode;

  private GetTokensForOwnerException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetTokensForOwnerException ownerNotFoundError(OwnerNotFoundError value) {
    return new GetTokensForOwnerException(new OwnerNotFoundErrorValue(value), 404);
  }

  public static GetTokensForOwnerException other(Object unknownValue, int statusCode) {
    return new GetTokensForOwnerException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class OwnerNotFoundErrorValue implements Value {
    ...
    public String toString() {
      return "GetTokensForOwnerException{" + "value: " + value + ", errorInstanceId: " + errorInstanceId + "}";
    }
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    @Override
    public String toString() {
      return "GetTokensForOwnerException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetTokensForOwnerException> {
    @Override
    public GetTokensForOwnerException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetTokensForOwnerException(value, statusCode);
    }
  }
}
