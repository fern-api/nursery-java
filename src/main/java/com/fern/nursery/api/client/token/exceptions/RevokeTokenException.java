...
@JsonDeserialize(
    using = RevokeTokenException.Deserializer.class
)
public final class RevokeTokenException extends Exception {
  private final Value value;

  private int statusCode;

  private RevokeTokenException(Value value, int statusCode) {
    this.value = value;
  ...
  public static RevokeTokenException tokenNotFoundError(TokenNotFoundError value) {
    return new RevokeTokenException(new TokenNotFoundErrorValue(value), 404);
  }

  public static RevokeTokenException other(Object unknownValue, int statusCode) {
    return new RevokeTokenException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class TokenNotFoundErrorValue implements Value {
    ...
    public String toString() {
      return "RevokeTokenException{" + "value: " + value + ", errorInstanceId: " + errorInstanceId + "}";
    }
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    @Override
    public String toString() {
      return "RevokeTokenException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<RevokeTokenException> {
    @Override
    public RevokeTokenException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new RevokeTokenException(value, statusCode);
    }
  }
}
