...
@JsonDeserialize(
    using = CreateMovieException.Deserializer.class
)
public final class CreateMovieException extends Exception {
  private final Value value;

  private int statusCode;

  private CreateMovieException(Value value, int statusCode) {
    this.value = value;
  ...
  public static CreateMovieException other(Object unknownValue, int statusCode) {
    return new CreateMovieException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "CreateMovieException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<CreateMovieException> {
    @Override
    public CreateMovieException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new CreateMovieException(value, statusCode);
    }
  }
}
